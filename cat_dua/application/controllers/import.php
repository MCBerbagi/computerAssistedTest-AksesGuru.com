    ...
    public function soal() {
        ...
        if($file[$length -1] == 'xlsx' || $file[$length -1] == 'xls') {
            ...
            for ($j = $idx_baris_mulai; $j <= $idx_baris_selesai; $j++) {
                ...
                $opsi_f = $_sheet->getCell("H".$j)->getCalculatedValue();
                $kunci = $_sheet->getCell("I".$j)->getCalculatedValue();

                if ($soal != "") {
                    $data[] = "('".$p['id_guru']."', '".$p['id_mapel']."', '".$bobot."', '".$soal."', '#####".$opsi_a."', '#####".$opsi_b."', '#####".$opsi_c."', '#####".$opsi_d."', '#####".$opsi_e."',  '#####".$opsi_f."', '".$kunci."', NOW(), 0, 0)"; 
                }
            }

            $strq = "INSERT INTO m_soal (id_guru, id_mapel, bobot, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, opsi_f, jawaban, tgl_input, jml_benar, jml_salah) VALUES ";
