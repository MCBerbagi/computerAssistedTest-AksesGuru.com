...
class Adm extends CI_Controller {
	...
	public function m_soal() {
		...
		$a['huruf_opsi'] = array("a","b","c","d","e","f");
		...
		if ($uri3 == "det") {
			...
			//ambil data awal
			$get_opsi_awal = $this->db->query("SELECT opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, opsi_f FROM m_soal WHERE id = '".$__id_soal."'")->row_array();
			...
			foreach ($gagal as $k => $v) {
				$arr_nama_file_upload = array("gambar_soal"=>"File Soal ", "gja"=>"File opsi A ", "gjb"=>"File opsi B ", "gjc"=>"File opsi C ", "gjd"=>"File opsi D ", "gje"=>"File opsi E ", "gjf"=>"File opsi F ");
			...
			$a['opsij'] = array(""=>"Jawaban","A"=>"A","B"=>"B","C"=>"C","D"=>"D","E"=>"E","F"=>"F");
			...
			if ($uri4 == 0) {
				$a['d'] = array("mode"=>"add","id"=>"0","id_guru"=>$id_guru,"id_mapel"=>"","bobot"=>"1","file"=>"","soal"=>"","opsi_a"=>"#####","opsi_b"=>"#####","opsi_c"=>"#####","opsi_d"=>"#####","opsi_e"=>"#####","opsi_f"=>"#####","jawaban"=>"","tgl_input"=>"");
			} else {
			...
			$nama_gambar = $this->db->query("SELECT id_mapel, file, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, opsi_f FROM m_soal WHERE id = '".$uri4."'")->row();
			...
			$pc_opsi_f = explode("#####", $nama_gambar->opsi_f);
			...
			@unlink("./upload/gambar_soal/".$pc_opsi_f[0]);
	...
	public function ikut_ujian() {
		...
		if ($uri3 == "simpan_satu") {
			...
			if ($cek_sdh_selesai < 1) {
				...
				if ($cek_sdh_ujian < 1)	{		
					$soal_urut_ok = array();
					$q_soal			= $this->db->query("SELECT id, file, tipe_file, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, opsi_f, '' AS jawaban FROM m_soal WHERE id_mapel = '".$cek_detil_tes->id_mapel."' AND id_guru = '".$cek_detil_tes->id_guru."' ".$acakan." LIMIT ".$cek_detil_tes->jumlah_soal)->result();
					$i = 0;
					foreach ($q_soal as $s) {
	...
	public function jvs() {
		...
		$data_soal 		= $this->db->query("SELECT id, gambar, soal, opsi_a, opsi_b, opsi_c, opsi_d, opsi_e, opsi_f FROM m_soal ORDER BY RAND()")->result();
